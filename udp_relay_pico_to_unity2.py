# ============================================
# UDP Relay with CSV Logging (Unity-safe)
...
print("UDP Relay started (Unity payload preserved)")
...
while True:
    ...
    for s in readable:
        ...
        if s is sock_from_pico:
            ...
            # 想定: finger_id, finger_norm, current
            if len(parts) >= 2:
                finger_id   = parts[0]
                finger_norm = parts[1]
                current     = parts[2] if len(parts) >= 3 else ""

                # ---- Unity へは従来フォーマットのみ送信 ----
                unity_payload = f"{finger_id},{finger_norm}"
                sock_to_unity.sendto(
                    unity_payload.encode("ascii"),
                    (UNITY_IP, UNITY_RECV_PORT)
                )
                ...
                print("Pico -> Unity :", unity_payload,
                      "(current logged:", current, ")")
            else:
                print("Invalid Pico payload :", text)
        ...
        # =====================================================
            ...
            finger_id = parts[0] if len(parts) >= 1 else ""
            force     = parts[1] if len(parts) >= 2 else ""
